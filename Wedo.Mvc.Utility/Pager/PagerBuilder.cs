...
namespace Wedo.Mvc.Pager
{
    ...
    internal class PagerBuilder
    {
        ...
        private string GenerateUrl(int pageIndex,bool isShowCurrent=false)
        {
            //若要生成url的页索引小于1或大于总页数或等于当前页索引时，无需生成分页导航链接
            if (pageIndex > _totalPageCount ||(isShowCurrent==false&& pageIndex == _pageIndex))
                return null;
        ...
        private MvcHtmlString CreateWrappedPagerElement(PagerItem item, string el)
        {
            ...
            switch (item.Type)
            {
                ...
                case PagerItemType.NumericPage:
                    ...
                    {//YT 20121215修改
                        string currentUrl = GenerateUrl(_pageIndex, true);
                        navStr =
                            string.Format(
                                (_pagerOptions.CurrentPagerItemWrapperFormatString ??
                                _pagerOptions.PagerItemWrapperFormatString)
                                .Replace("{url}", currentUrl), el);
                    }
